Implement exactly the plan below in this repository. Respect the existing code
style. Only touch the files listed in the plan. Do not commit. Do not touch
OS/stepper-os. After your edits, run `cargo clippy -p omega -p omega-core -- -D warnings`
yourself and fix every remaining warning until it is clean, then run
`cargo test -p omega-core --lib` and make sure it is green. The bridge will
re-run both as the final verify.
