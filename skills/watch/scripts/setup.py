...
OMEGA_SECRETS_FILE = Path.home() / ".omega" / "secrets" / "integrations.env"
...
def _read_key_file(path: Path, name: str) -> str | None:
    """Parse KEY=value from an env-style file (comments and quotes tolerated)."""
    if not path.exists():
        return None
    try:
...
def _read_env_key(name: str) -> str | None:
    ...
    return _read_key_file(CONFIG_FILE, name)


def _have_api_key() -> tuple[bool, str | None]:
    """Whisper key lookup chain: env > ~/.config/watch/.env > ~/.omega/secrets/integrations.env."""
    for key_name, backend in (("GROQ_API_KEY", "groq"), ("OPENAI_API_KEY", "openai")):
        if _read_env_key(key_name) or _read_key_file(OMEGA_SECRETS_FILE, key_name):
            return True, backend
    return False, None
...
def _install_hint_linux(missing: list[str]) -> str:
    ...
    hints = [
        "Ubuntu/VPS: `sudo apt-get install -y ffmpeg pipx && pipx install yt-dlp`",
        'For YouTube on servers/datacenter IPs see SKILL.md "YouTube on servers" (deno + yt-dlp-ejs + PoT provider + cookies).',
    ]
    return "\n  ".join(hints)
