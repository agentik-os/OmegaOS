...
if [[ -d "$OMEGA_SRC/scripts/hooks" ]]; then
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        # DELIBERATELY NOT REGISTERED FOR CODEX — measured 2026-07-24.
        #
        # Codex exposes the same hook events, but writing new entries into
        # ~/.codex/hooks.json arms a blocking startup prompt ("N hooks are new
        # or changed") whose per-hook `trusted_hash` is recorded in
        # ~/.codex/config.toml [hooks.state]. A DISPATCHED oracle or worker is
        # non-interactive, so it hangs on that prompt forever — and since
        # config.agent_command defaults to codex, registering these hooks broke
        # EVERY dispatch until the prompt was answered by hand. The hash is not
        # reproducible across machines or Codex versions, so install.sh cannot
        # pre-trust them, and `--dangerously-bypass-hook-trust` would let ANY
        # repo-local hooks.json run unvetted (the supply-chain surface
        # R-REPO-INSTALL exists to guard).
        #
        # Codex therefore gets the doctrine through ~/.omega/AGENTS.md (the full
        # 49 Laws + Rules, generated by `omega sync`), which needs no trust
        # handshake — verified live: a fresh `codex exec` names L6 and quotes
        # rules that were never in the OMEGA.md summary. It gets the CONTRACT,
        # not the hook-level ENFORCEMENT. To opt in on one machine, run `codex`
        # once interactively and accept the hook review after adding them.
    else
