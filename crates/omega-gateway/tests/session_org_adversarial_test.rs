...
async fn n_concurrent_puts_on_distinct_keys_never_lose_an_update() {
    ...
    for i in 0..N {
        let name = format!("oracle-Concurrent-{i}");
        assert_eq!(
            entries[&name]["label"], format!("label-{i}"),
...
async fn corrupted_session_org_json_degrades_to_empty_map_no_panic() {
    ...
    assert_eq!(res.status(), 200, "corrupted session_org.json must not surface as a 500 or crash");
    let body: serde_json::Value = res.json().await.unwrap();
    assert_eq!(body["entries"], serde_json::json!({}), "corrupted file must degrade to an empty overlay");
}
...
async fn corrupted_session_org_json_does_not_block_a_subsequent_put() {
    ...
    let res = reqwest::Client::new().get(format!("{base}/v1/session-org")).bearer_auth(&token).send().await.unwrap();
    let body: serde_json::Value = res.json().await.unwrap();
