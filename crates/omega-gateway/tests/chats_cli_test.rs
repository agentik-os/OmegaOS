...
fn run_chats(gateway_dir: &std::path::Path) -> String {
    ...
    assert!(output.status.success(), "chats command exited non-zero: {output:?}");
...
fn chats_command_reports_none_when_empty() {
    ...
    assert!(stdout.contains("no chats"), "expected 'no chats', got: {stdout}");
}
...
fn chats_command_lists_created_chat() {
    ...
    assert!(stdout.contains("hello world"), "expected title in output: {stdout}");
    assert!(stdout.contains("claude"), "expected agent in output: {stdout}");
}
