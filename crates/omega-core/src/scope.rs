...
impl ScopeClaim {
    ...
    pub(crate) fn validate_authority(&self) -> Result<()> {
        self.validate_for_path(&self.session)?;
        if self.workspace_id.is_none() || self.claim_id.is_none() {
            bail!("authoritative scope claim lacks workspace or generation identity");
        }
        Ok(())
    }
...
pub(crate) fn canonical_workspace_identity(workspace: &Path) -> Result<String> {
...
/// Stable V3 lease resource for one normalized selector in one canonical
/// workspace. Compatibility claims and the ledger both derive lease identity
/// here so neither side can silently drift to a different resource set.
pub(crate) fn lease_resource_key(workspace_id: &str, normalized_selector: &str) -> String {
