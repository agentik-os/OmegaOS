...
> Spot movement early and turn it into a named opportunity.

**Suite position:** `13` in **02 · DISCOVER & DECIDE** (Find what is worth building).

## What this OS is for

Trend & Opportunity {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: spot movement early and turn it into a named opportunity.
...
```bash
agentik install trend-opportunity-os          # install this OS
agentik configure trend-opportunity-os        # answer the minimum setup questions
agentik run trend-opportunity-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`13. Trend & Opportunity {OS}`.
