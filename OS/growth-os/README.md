...
> Loops, experiments and the channels that compound.

**Suite position:** `37` in **04 · GROW** (Turn value into distribution and revenue).

## What this OS is for

Growth {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: loops, experiments and the channels that compound.
...
```bash
agentik install growth-os          # install this OS
agentik configure growth-os        # answer the minimum setup questions
agentik run growth-os              # start using it
```
