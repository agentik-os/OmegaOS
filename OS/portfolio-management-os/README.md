...
> Run the portfolio after the deal: reporting, support, reallocation.

**Suite position:** `62` in **07 · CAPITAL** (Turn assets into capital allocation).

## What this OS is for

Portfolio Management {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: run the portfolio after the deal: reporting, support, reallocation.
...
```bash
agentik install portfolio-management-os          # install this OS
agentik configure portfolio-management-os        # answer the minimum setup questions
agentik run portfolio-management-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`62. Portfolio Management {OS}`.
