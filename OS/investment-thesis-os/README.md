...
**Suite position:** `57` in **07 · CAPITAL** (Turn assets into capital allocation).

## What this OS is for

Investment Thesis {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: write the thesis before the cheque, and test it after.
...
```bash
agentik install investment-thesis-os          # install this OS
agentik configure investment-thesis-os        # answer the minimum setup questions
agentik run investment-thesis-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`57. Investment Thesis {OS}`.
