...
> Make hard calls well: framing, reversibility, evidence, the decision record.

**Suite position:** `08` in **01 · PERSONAL** (Operate yourself).

## What this OS is for

Decision {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: make hard calls well: framing, reversibility, evidence, the decision record.
...
```bash
agentik install decision-os          # install this OS
agentik configure decision-os        # answer the minimum setup questions
agentik run decision-os              # start using it
```
