...
**Suite position:** `61` in **07 · CAPITAL** (Turn assets into capital allocation).

## What this OS is for

Deal Structuring {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: terms, instruments and incentives that survive contact with reality.
...
```bash
agentik install deal-structuring-os          # install this OS
agentik configure deal-structuring-os        # answer the minimum setup questions
agentik run deal-structuring-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`61. Deal Structuring {OS}`.
