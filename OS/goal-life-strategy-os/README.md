...
> Life-level goals and the strategy that makes them reachable.

**Suite position:** `04` in **01 · PERSONAL** (Operate yourself).

## What this OS is for

Goal & Life Strategy {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: life-level goals and the strategy that makes them reachable.
...
```bash
agentik install goal-life-strategy-os          # install this OS
agentik configure goal-life-strategy-os        # answer the minimum setup questions
agentik run goal-life-strategy-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`04. Goal & Life Strategy {OS}`.
