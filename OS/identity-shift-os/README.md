...
**Suite position:** `02` in **01 · PERSONAL** (Operate yourself).

## What this OS is for

Identity Shift {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: deliberate identity change: who you must become before what you must do.
...
```bash
agentik install identity-shift-os          # install this OS
agentik configure identity-shift-os        # answer the minimum setup questions
agentik run identity-shift-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`02. Identity Shift {OS}`.
