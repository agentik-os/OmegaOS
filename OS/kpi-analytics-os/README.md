...
> Measure the few numbers that actually change decisions.

**Suite position:** `48` in **05 · OPERATE** (Turn chaos into repeatable execution).

## What this OS is for

KPI & Analytics {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: measure the few numbers that actually change decisions.
...
```bash
agentik install kpi-analytics-os          # install this OS
agentik configure kpi-analytics-os        # answer the minimum setup questions
agentik run kpi-analytics-os              # start using it
```
