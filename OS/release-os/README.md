...
**Suite position:** `27` in **03 · BUILD** (Turn evidence into products).

## What this OS is for

Release {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: ship it: release boundaries, rollout, rollback and the incident path.
...
```bash
agentik install release-os          # install this OS
agentik configure release-os        # answer the minimum setup questions
agentik run release-os              # start using it
```
