...
> Talk to real users and extract what they actually need.

**Suite position:** `17` in **02 · DISCOVER & DECIDE** (Find what is worth building).

## What this OS is for

Customer Discovery {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: talk to real users and extract what they actually need.
...
```bash
agentik install customer-discovery-os          # install this OS
agentik configure customer-discovery-os        # answer the minimum setup questions
agentik run customer-discovery-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`17. Customer Discovery {OS}`.
