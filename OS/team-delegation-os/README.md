...
**Suite position:** `47` in **05 · OPERATE** (Turn chaos into repeatable execution).

## What this OS is for

Team & Delegation {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: hand work off so it comes back right the first time.
...
```bash
agentik install team-delegation-os          # install this OS
agentik configure team-delegation-os        # answer the minimum setup questions
agentik run team-delegation-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`47. Team & Delegation {OS}`.
