...
> The strategy of the business as an asset, not as a job.

**Suite position:** `54` in **06 · OWN** (Turn work into assets).

## What this OS is for

Business Strategy {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: the strategy of the business as an asset, not as a job.
...
```bash
agentik install business-strategy-os          # install this OS
agentik configure business-strategy-os        # answer the minimum setup questions
agentik run business-strategy-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`54. Business Strategy {OS}`.
