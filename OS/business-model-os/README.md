...
> How value is created, delivered and captured, made explicit.

**Suite position:** `19` in **02 · DISCOVER & DECIDE** (Find what is worth building).

## What this OS is for

Business Model {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: how value is created, delivered and captured, made explicit.
...
```bash
agentik install business-model-os          # install this OS
agentik configure business-model-os        # answer the minimum setup questions
agentik run business-model-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`19. Business Model {OS}`.
