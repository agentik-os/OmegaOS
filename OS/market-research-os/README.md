...
> Market and customer evidence, and the validation that follows.

**Suite position:** `16` in **02 · DISCOVER & DECIDE** (Find what is worth building).

## What this OS is for

Market Research {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: market and customer evidence, and the validation that follows.
...
```bash
agentik install market-research-os          # install this OS
agentik configure market-research-os        # answer the minimum setup questions
agentik run market-research-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`16. Market Research {OS}`.
