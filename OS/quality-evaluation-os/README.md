...
**Suite position:** `25` in **03 · BUILD** (Turn evidence into products).

## What this OS is for

Quality & Evaluation {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: independent certification of what was built, before it ships.
...
```bash
agentik install quality-evaluation-os          # install this OS
agentik configure quality-evaluation-os        # answer the minimum setup questions
agentik run quality-evaluation-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`25. Quality & Evaluation {OS}`.
