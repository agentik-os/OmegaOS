...
> Install, configure, run, compose, update and evaluate every Agentik OS.

**Suite position:** `00` in **00 · RUNTIME** (Run the entire Agentik ecosystem).

## What this OS is for

Agentik Runtime {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: install, configure, run, compose, update and evaluate every agentik os.
...
```bash
agentik install agentik-runtime          # install this OS
agentik configure agentik-runtime        # answer the minimum setup questions
agentik run agentik-runtime              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`00. Agentik Runtime {OS}`.
