...
**Suite position:** `55` in **06 · OWN** (Turn work into assets).

## What this OS is for

Exit & Liquidity {OS} is one operative system of the AGENTIK {OS} suite. It is not a
...
Use it when your problem is: prepare, time and run a liquidity event.
...
```bash
agentik install exit-liquidity-os          # install this OS
agentik configure exit-liquidity-os        # answer the minimum setup questions
agentik run exit-liquidity-os              # start using it
```

Inside OmegaOS it is also reachable from `omega menu`, **OS** tab, entry
`55. Exit & Liquidity {OS}`.
